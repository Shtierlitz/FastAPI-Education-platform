...
from api.models import ShowUser, UserCreate, DeleteUserResponse, UpdateUserRequest, UpdatedUserResponse
...
async def _update_user(user_id: UUID, updated_user_params: dict, db: AsyncSession) -> Union[UUID, None]:
    async with db as session:
        async with session.begin():
            user_dal = UserDAL(session)
